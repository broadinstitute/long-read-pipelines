...
UMI_LENGTH = 10
PRE_CELL_BARCODE_SEQUENCE = "TCTACACGACGCTCTTCCGATCT"
...
def print_alignment(seq, adapter_seq, query_start, query_end, ref_start, ref_end, post_alignment_start, umi_length, cigar_ops, num_insertions, num_deletions, post_umi_sequence):
...
def main(bam_filename, out_file_name, pre_barcode_seq, cell_barcode_tag, umi_length, new_umi_tag, existing_umi_tag,
         post_umi_sequence, short_reads_umi_file, ssw_path):
    ...
    with pysam.AlignmentFile(bam_filename, 'rb', check_sq=False, require_index=False) as bam_file, \
        ...
        with pysam.AlignmentFile(out_file_name, 'wb', header=bam_file.header) as out_bam_file:
            ...
            for read in bam_file:
                ...
                adapter_seq = pre_barcode_seq + read.get_tag(cell_barcode_tag)
                ...
                post_alignments = get_alignment(ssw,
                                                seq[alignment_end:alignment_end + 2 * (umi_length + len(post_umi_sequence))],
                                                post_umi_sequence,
                                                alphabet,
                                                letter_to_int,
                                                mat)
                ...
                # Debugging:
                # print_alignment(seq[:num_read_bases_to_align + (2 * (umi_length + len(post_umi_sequence)))],
                #                 adapter_seq, res.nQryBeg, res.nQryEnd, res.nRefBeg,
                #                 res.nRefEnd, post_alignment_start, umi_length, alignment_cigar,
                #                 alignment_num_insertions, alignment_num_deletions, post_umi_sequence)
                print(marker_string)
                print(read_string)
                ...
                read.set_tag(BACK_ALIGNMENT_SCORE_TAG, f"{post_alignments[3].nScore}/{len(post_umi_sequence) * match_score}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--pre-barcode-seq', help='Sequence preceding the cell barcode used in the library prep.  (default: %(default)s)', type=str,
        default=PRE_CELL_BARCODE_SEQUENCE
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--post-umi-sequence', help='Sequence just after the UMI.  (default: %(default)s)', type=str,
        default=POST_UMI_SEQ
    )

    args = parser.parse_args()

    main(args.bam, args.output_file, args.pre_barcode_seq, args.cell_barcode_tag, args.umi_length, args.new_umi_tag,
         args.existing_umi_tag, args.post_umi_sequence, args.short_reads_umis, args.ssw_path)
