...
UMI_LENGTH = 10
PRE_UMI_SEQ = "TCTACACGACGCTCTTCCGATCT"
...
def build_alignment_display_strings(seq, adapter_seq,
    ...
    query_string += post_umi_sequence[:alignment_b_start_boreas]
    ...
    read_string, alignment_string, query_string, read_offset, adapter_offset, refined_cigar_ops_b = \
        _build_alignment_strings_from_cigar(seq, post_umi_sequence, cigar_ops_b,
                                            read_offset, alignment_b_start_boreas,
                                            read_string, alignment_string, query_string)

    # Let's pad out the remaining bases a bit:
    context_bases = 20
    remaining_adapter_bases = len(post_umi_sequence) - adapter_offset
    read_string += seq[read_offset:read_offset + remaining_adapter_bases + context_bases]
    query_string += post_umi_sequence[adapter_offset:adapter_offset+remaining_adapter_bases + context_bases]
...
def print_alignment(seq, adapter_seq, query_start, query_end, ref_start, ref_end, post_alignment_start, umi_length,
                    cigar_ops, num_insertions, num_deletions, post_umi_sequence):
...
def main(bam_filename, out_file_name, short_reads_umi_file, ssw_path,
         pre_pre_umi_seq, pre_umi_tag, pre_umi_seq,
         umi_length, existing_umi_tag,
         post_umi_tag, post_umi_seq,
         new_umi_tag):
    ...
    num_reads_missing_5p = 0
    num_ccs_reads_missing_5p = 0
    num_clr_reads_missing_5p = 0
    ...
    with pysam.AlignmentFile(bam_filename, 'rb', check_sq=False, require_index=False) as bam_file, \
        ...
        with pysam.AlignmentFile(out_file_name, 'wb', header=bam_file.header) as out_bam_file:
            ...
            for read in bam_file:
                ...
                # TODO: THRESHOLD SHOULD BE AN INPUT ARGUMENT
                is_ccs = read.get_tag("rq") > 0
                num_read_bases_to_align = NUM_BASES_TO_ALIGN_CCS if is_ccs else NUM_BASES_TO_ALIGN_CLR

                # Create the full pre-umi sequence:
                full_pre_umi_sequence = read.get_tag(pre_umi_tag) if pre_umi_tag else pre_umi_seq
                if pre_pre_umi_seq:
                    full_pre_umi_sequence = pre_pre_umi_seq + full_pre_umi_sequence
                ...
                # Now we align the post_umi_sequence to the region following our adapters
                # so we can have a bounded region within which we can grab the UMI:

                # Create the full post-umi sequence:
                full_post_umi_sequence = read.get_tag(post_umi_tag) if post_umi_tag else post_umi_seq

                post_alignments = get_alignment(ssw,
                                                seq[alignment_end:
                                                    alignment_end + 2 * (umi_length + len(full_post_umi_sequence))],
                                                full_post_umi_sequence,
                                                alphabet,
                                                letter_to_int,
                                                mat)
                ...
                read_string, alignment_string, query_string, marker_string, refined_cigar, refined_post_alignment_cigar = \
                    build_alignment_display_strings(seq, full_pre_umi_sequence,
                ...
                # Debugging:
                # print_alignment(seq[:num_read_bases_to_align + (2 * (umi_length + len(full_post_umi_sequence)))],
                #                 full_pre_umi_sequence, res.nQryBeg, res.nQryEnd, res.nRefBeg,
                #                 res.nRefEnd, post_alignment_start, umi_length, alignment_cigar,
                #                 alignment_num_insertions, alignment_num_deletions, full_post_umi_sequence)
                print(marker_string)
                print(read_string)
                ...
                try:
                    new_tenx_start_offsets[alignment_start - segments['5p_Adapter'][0]] += 1
                    print(f"10x start offset: {alignment_start - segments['5p_Adapter'][0]}")
                except KeyError:
                    num_reads_missing_5p += 1
                    print("Missing 10x adapter")
                    if is_ccs:
                        num_ccs_reads_missing_5p += 1
                    else:
                        num_clr_reads_missing_5p += 1
                ...
                try:
                    ## TODO: THIS SHOULD BE MODEL-DEPENDENT.  YET ANOTHER REASON TO ROLL THIS INTO LONGBOW.
                ...
                read.set_tag(FRONT_ALIGNMENT_SCORE_TAG, f"{res.nScore}/{len(full_pre_umi_sequence) * match_score}")
                read.set_tag(BACK_ALIGNMENT_SCORE_TAG, f"{post_alignments[3].nScore}/{len(full_post_umi_sequence) * match_score}")
    ...
    print()
    print_single_count("Num reads missing 10x adapter (5p_Adapter)", num_reads_missing_5p, num_reads)
    print_single_count("Num CCS reads missing 10x adapter (5p_Adapter)", num_ccs_reads_missing_5p, num_reads, num_ccs_reads, " of CCS")
    print_single_count("Num CLR reads missing 10x adapter (5p_Adapter)", num_clr_reads_missing_5p, num_reads, num_clr_reads, " of CLR")
    print()
...
if __name__ == '__main__':
    ...
    # Arguments for the marker sequences around the UMI:
    parser.add_argument(
        '--pre-pre-umi-seq', help='Fixed sequence occuring before the pre-umi sequence.', type=str
    )

    pre_umi_group = parser.add_mutually_exclusive_group(required=True)
    pre_umi_group.add_argument(
        '--pre-umi-tag', help='Read tag containing the sequence that occurs just before the UMI.', type=str
    )
    pre_umi_group.add_argument(
        '--pre-umi-seq', help='Sequence preceding the cell barcode used in the library prep.  (default: %(default)s)', type=str,
        default=PRE_UMI_SEQ
    )
    ...
    post_umi_group = parser.add_mutually_exclusive_group(required=True)
    post_umi_group.add_argument(
        '--post-umi-tag', help='Read tag containing the sequence that occurs just after the UMI.', type=str
    )
    post_umi_group.add_argument(
        '--post-umi-seq', help='Sequence occuring just after the UMI in the read.  (default: %(default)s)', type=str,
        default=POST_UMI_SEQ
    )
    ...
    main(args.bam, args.output_file, args.short_reads_umis, args.ssw_path,
         args.pre_pre_umi_seq, args.pre_umi_tag, args.pre_umi_seq,
         args.umi_length, args.existing_umi_tag,
         args.post_umi_tag, args.post_umi_seq,
         args.new_umi_tag)
