...
def print_logo(use_mosiac_aligner=True):
...
def compute_detailed_alignment_info(
    ...
    # TODO: See the extract tool for an alternate version of this segment.  This may be fine, but may have a bug.
...
def find_all_ordered_seqs_in_alignment_results(seq_order, processed_results):
    ...
    for i, result in enumerate(processed_results):
        if result.seq_name == seq_order[seq_index]:
...
def cartographize(args):
    ...
    structural_alignment_type = AlignmentAlgorithm.MOSAIC_ALIGNER if args.MOSAIC else AlignmentAlgorithm.TESSERAE
    ...
    with open(args.outfile, 'w') as out_file:
        ...
        with ReadFile(args.reads) as reads_file:
            ...
            for read_num, read_sequence in enumerate(reads_file.get_reads()):
                ...
                # Check the Reverse Complemented direction if forward doesn't work:
                if len(index_dict_list) == 0:
                    LOGGER.info("Forward direction produced no good results.  Trying reverse direction.")
                    is_forward_direction = False
                    read_sequence_rc = Sequence(read_sequence.name + "_RC", reverse_complement(read_sequence.sequence))

                    ordered_segment_alignment_results, query_result = align_sequences(
                        read_sequence_rc, ordered_sequences, args.minqual, args.minbases, p_rec=P_REC_KNOWN,
                        create_plots=args.plot, plot_folder=args.plot_folder, extra_plot_suffix="structural",
                        alignment_type=structural_alignment_type, threads=num_threads
                    )

                    if len(ordered_segment_alignment_results) == 0:
                        LOGGER.info("%sThis read has no filtered alignments for the known ordered sequences.",
                                    spacing_one)
                    else:
                        index_dict_list = find_all_ordered_seqs_in_alignment_results(
                            sequence_order, ordered_segment_alignment_results
                ...
                if len(index_dict_list) == 0:
                    ...
                    for result_index_dict in index_dict_list:
                        ...
                        if is_forward_direction:
                            num_good_forward_alignments += 1
                        else:
                            num_good_reverse_complement_alignments += 1

            LOGGER.info("Processed %d reads.", num_reads)
            LOGGER.info("# Forward reads with good structural alignments: %d", num_good_forward_alignments)
            LOGGER.info("# RC reads with good structural alignments: %d", num_good_reverse_complement_alignments)
            LOGGER.info("Total # reads with good structural alignments: %d",
                        num_good_forward_alignments + num_good_reverse_complement_alignments)
...
def write_entry_to_output_file(
    ...
    :param is_forward_direction: Boolean indicating if the read is in the forward direction.
    ...
    # Add label for strandedness:
    if is_forward_direction:
        direction = "FORWARD"
    else:
        direction = "REVERSE_COMPLEMENTED"

    out_file.write(f"\t{direction}")
...
def main(raw_args):
    ...
    parser = argparse.ArgumentParser(
        description="Ingests three files: one read file(FASTA/FASTQ/SAM/BAM) containing reads to be mapped, "
    ...
    align_required_args.add_argument(
        "-r", "--reads", help="Reads FASTA file.", required=True
    )
    align_required_args.add_argument(
        "-s", "--segments", help="Segments FASTA file.", required=True
    )
    align_required_args.add_argument(
    ...
    parser.add_argument(
        "-M",
        "--MOSAIC",
        help="Use MosaicAligner, rather than Tesserae to align.",
        action="store_true"
    )
