...
# Define a regex to use to determine if a node is a read or not:
mas_seq_node_name_regex = re.compile(r"""^m[0-9]*e?_[0-9]{6}_[0-9]{6}""")


def gencode_node_filter(n):
    return n.startswith('ENST')


def stringtie2_node_filter(n):
    return n.startswith('STRG')


def mas_seq_node_filter(n):
    return mas_seq_node_name_regex.match(n)


def gencode_gene_name_filter(n):
    return not stringtie2_node_filter(n) and not mas_seq_node_filter(n)
