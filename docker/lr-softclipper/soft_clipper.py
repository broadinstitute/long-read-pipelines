...
parser.add_argument('--clipping-threshold', dest='clipping_threshold', type=int, required=True,
                    help='Minimum soft clip length for read splitting')
parser.add_argument('--split-read-prefix', dest='split_read_prefix', type=str, required=True,
                    help='Split reads will be prefixed with this arguments.'
                         'Ex. read_name = read123, split-read-prefix = x1'
                         'would produce read123_x1-l for a split on the left side'
                         'and read_123_x1-r for a split on the right side.'
                         'IMPORTANT - this value should be different on successive rounds '
                         'of splitting so that there are no duplicate naming conflicts.')
...
split_read_prefix = args.split_read_prefix
...
input_alignments = pysam.AlignmentFile("-", "r")
...
if using_ref:
    ...
    if write_ref_conflicts:
        unassisted_bam = pysam.AlignmentFile(f"{write_ref_conflicts_prefix}_unassisted.bam", "wb", template=input_alignments)
        ref_bam = pysam.AlignmentFile(f"{write_ref_conflicts_prefix}_ref.bam", "wb", template=ref_alignments_file)
    

for line in input_alignments:
    if not is_primary(line.flag):
        continue
    ...
    if left_clip_len >= CLIPPING_THRESHOLD:
        if using_ref and abs(ref_left_clip_len - left_clip_len) >= REF_DIFF_THRESHOLD:
            ...
            print(f"@{read_id}_{split_read_prefix}-l\n"
                  f"{seq[:left_clip_len]}\n"
                  f"+\n"
    ...
    if right_clip_len >= CLIPPING_THRESHOLD:
        if using_ref and abs(ref_right_clip_len - right_clip_len) >= REF_DIFF_THRESHOLD:
            ...
            print(f"@{read_id}_{split_read_prefix}-r\n"
