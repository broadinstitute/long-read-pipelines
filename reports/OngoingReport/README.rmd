...
```{r gs_get}
gs_get <- function(gspath, dir="scratch", use_cache=TRUE) {
    ...
    if (!file.exists(outpath) | !use_cache) {
...
get_workflow_metadata <- function(cromwell_hash) {
    #return(fromJSON(readLines(curl(paste("https://cromwell-v36.dsde-methods.broadinstitute.org/api/workflows/v1/", cromwell_hash, "/metadata?expandSubWorkflows=false", sep="")), warn=FALSE)))
...
for (i in 1:nrow(eap_stats)) {
    rlu = read_delim(rlu_files[[i]], delim=' ', col_names=c("zmw", "range", "mq", "length"))
...
plot(0, 0, type="n", xlim=c(0, 20000), ylim=c(0, max(unlist(lapply(rl_h_list, max)))), bty="n", las=1, ylab="", xlab="Read length (bp)", cex.axis=1.3, cex.lab=1.3)
...
```{r yield}
eap_stats$new_sample_name = (eap_stats %>% pull(sample_name) %>% gsub("RGP_94_1", "clinical sample 1", .) %>% gsub("RGP_94_2_LowYield", "clinical sample 2", .) %>% gsub("RGP_94_2_Rework", "clinical sample 2 (rework)", .) %>% gsub("RGP_94_3_LowYield", "clinical sample 3", .) %>% gsub("RGP_94_3_Rework", "clinical sample 3 (rework)", .) %>% gsub("VS_ABC012_3_1_mother", "clinical sample 4", .) %>% gsub("VS_ABC012_1_1_proband", "clinical sample 5", .) %>% gsub("VS_ABC012_2_1_father", "clinical sample 6", .) %>% gsub("HCC1954T", "tumor cell line 1", .) %>% gsub("HCC1954N", "paired normal cell line 1", .) %>% gsub("Talkowski_MIN36192_SM-IBA3O", "clinical sample 7", .) %>% gsub("Talkowski_MIN39144_SM-IBA3L", "clinical sample 8", .) %>% gsub("Talkowski_JFG36815F_SM-IIU4J", "clinical sample 9", .) %>% gsub("Talkowski_22701_SM-IIU4K", "clinical sample 10", .))
...
par(mar=c(9, 6, 2, 1))
barplot(eap_stats$yield[sl.indices]/1e9, names.arg = eap_stats$new_sample_name[sl.indices], las=2, cex.names=0.5, ylim=c(0, max(eap_stats$yield))/1e9, col=c("#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#F28379", "#05C7F2","#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#D94436", "#05C7F2", "#D94436", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#D94436", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#D94436"))
abline(h=median(eap_stats$yield), lty=2, col="red")
