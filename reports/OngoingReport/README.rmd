...
sslogs = system("/Users/kiran/google-cloud-sdk/bin/gsutil ls gs://broad-dsde-methods-kiran/workflow_output/CorrectAndAlignWorkflow/*/call-SplitSubreads/SplitSubreads.log", intern=TRUE)
...
for (sslog in sslogs) {
    ...
    merged_uncorrected_size = ifelse(identical(merged_uncorrected_size, character(0)), 0, merged_uncorrected_size)
    merged_corrected_size = ifelse(identical(merged_corrected_size, character(0)), 0, merged_corrected_size)
    
    run_type = ifelse(grepl("_CLR", sample_name), "CLR", "CCS")
    
    m = rbind(m, c(cromwell_hash, codec, compression, approx_reads_per_shard, movie_name, sample_name, run_type, merged_uncorrected_bam, merged_uncorrected_size, merged_corrected_bam, merged_corrected_size))
}

colnames(m) = c("cromwell_hash", "codec", "compression", "shard_size", "movie_name", "sample_name", "run_type", "merged_uncorrected_bam", "merged_uncorrected_size", "merged_corrected_bam", "merged_corrected_size")
tm = as_tibble(m)

tm$accept = TRUE
tm[which(tm$cromwell_hash %in% c("66182c6f-97e7-4b93-93b4-1ee830ce3ad5", "c31f1abe-84f7-4259-a5ef-7b7962d5cb3e", "fb6c81c9-c3df-4d49-a3ce-aa6ae0f6e45d", "5199a7e9-4579-4ede-976d-8b74cb211163", "ba5a752a-ac77-456c-99e9-2d99c8cb098f")), "accept"] = FALSE
```
...
cromwell_hash = tm$cromwell_hash[1]
for (cromwell_hash in tm$cromwell_hash) {
...
```{r costs_boxplot}
hashes.clr = tm %>% filter(sample_name != "SmallTestData", sample_name != "Ecoli_Training_Run") %>% filter(grepl("CLR", sample_name)) %>% pull(cromwell_hash)
hashes.ccs = tm %>% filter(sample_name != "SmallTestData", sample_name != "Ecoli_Training_Run") %>% filter(!grepl("CLR", sample_name)) %>% pull(cromwell_hash)
