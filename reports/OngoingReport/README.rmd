...
```{r gs_get}
gs_get <- function(gspath, dir="scratch") {
    gsdir = gsub("gs://", "", dirname(gspath))
    gsbase = basename(gspath)
    
    outdir = paste(dir, gsdir, sep="/")
    outpath = paste(outdir, gsbase, sep="/")
    ...
    if (!file.exists(outpath)) {
        system(paste("/Users/kiran/google-cloud-sdk/bin/gsutil cp", gspath, outpath))
    }
...
gs_cat <- function(file, pipe="") {
    local_file = gs_get(file)
    
    return(system(paste("cat", local_file, ifelse(pipe == "", "", "|"), pipe), intern=TRUE))
}
```
...
```{r gs_read_table}
gs_read_table <- function(table_file, sep="\t", header=FALSE) {
    q = gs_cat(table_file)
    qa = grep("#|^$", q, value=TRUE, invert=TRUE)
    con = textConnection(qa)
    f = read.table(con, sep=sep, header=header)
    close(con)
...
get_splitsubreads_stats <- function(cromwell_hash) {
    l = gs_best(gs_ls(paste("gs://broad-dsde-methods-kiran/workflow_output/*/", cromwell_hash, "/call-SplitSubreads/**SplitSubreads.log", sep="")))
    
    if (gs_exists(l$file)) {
        sslog = l$file
        timestamp = l$timestamp
        cmd = gs_cat(sslog, pipe="grep -m1 java")
        ...
        q = gs_cat(sslog, pipe="grep '.bam'") %>% tail(n=1) %>% strsplit("[.\\s]+", perl=TRUE) %>% unlist()
...
get_ccs_report <- function(cromwell_hash) {
    x = paste("gs://broad-dsde-methods-kiran/workflow_output/*/", cromwell_hash, "/call-MergeCCSReports/**ccs_report.txt", sep="")
    xa = gs_best(gs_ls(x))
...
get_read_lengths_uncorrected_file <- function(cromwell_hash) {
    rlfile = paste("gs://broad-dsde-methods-kiran/workflow_output/*/", cromwell_hash, "/call-ReadLengthsUncorrected/**.readlengths.txt", sep="")
    rl = gs_ls(rlfile)
    return(gs_best(rl))
}
```
...
get_read_lengths_corrected_file <- function(cromwell_hash) {
    rlfile = paste("gs://broad-dsde-methods-kiran/workflow_output/*/", cromwell_hash, "/call-ReadLengthsCorrected/**.readlengths.txt", sep="")
    rl = gs_ls(rlfile)
    return(gs_best(rl))
}
```
...
```{r get_workflow_metadata}
get_workflow_metadata <- function(cromwell_hash) {
    con = curl(paste("https://cromwell-v36.dsde-methods.broadinstitute.org/api/workflows/v1/", cromwell_hash, "/metadata?expandSubWorkflows=false", sep=""))
    fa = readLines(con, warn=FALSE)
    q = fromJSON(fa)
    close(con)
...
get_bam_uncorrected_file <- function(cromwell_hash) {
    bam_file = paste("gs://broad-dsde-methods-kiran/workflow_output/*/", cromwell_hash, "/call-MergeUncorrected/**.bam", sep="")
...
get_bam_corrected_file <- function(cromwell_hash) {
    bam_file = paste("gs://broad-dsde-methods-kiran/workflow_output/*/", cromwell_hash, "/call-MergeCorrected/**.bam", sep="")
    return(gs_best(gs_ls(bam_file)))
}
```
...
get_bam_remaining_file <- function(cromwell_hash) {
    bam_file = paste("gs://broad-dsde-methods-kiran/workflow_output/*/", cromwell_hash, "/call-MergeRemaining/**.bam", sep="")
    return(gs_best(gs_ls(bam_file)))
}
```
...
get_alignment_stats <- function(cromwell_hash, type) {
    tbl_file = paste("gs://broad-dsde-methods-kiran/workflow_output/*/", cromwell_hash, "/call-AlignmentStats", type, "/**.alignment.report.txt", sep="")
    
    tbl = gs_best(gs_ls(tbl_file))
    
    if (gs_exists(tbl$file)) {
        return(gs_read_table(tbl$file, header=TRUE))
    }
...
get_workflow_stats <- function(cromwell_hash) {
    ...
    rl_uncorrected_file = get_read_lengths_uncorrected_file(cromwell_hash)
    rl_corrected_file = get_read_lengths_corrected_file(cromwell_hash)
    ...
    names(alignment_stats_uncorrected) = paste(names(alignment_stats_corrected), "UNCORRECTED", sep="_")
    names(alignment_stats_corrected) = paste(names(alignment_stats_corrected), "CORRECTED", sep="_")
    names(alignment_stats_remaining) = paste(names(alignment_stats_corrected), "REMAINING", sep="_")
    
    return(c(list("cromwell_hash" = cromwell_hash),
             workflow_status = get_workflow_execution_status(cromwell_hash),
             splitsubreads_stats,
             alignment_stats_uncorrected,
             alignment_stats_corrected,
             alignment_stats_remaining,
             ccs_stats,
             file_read_lengths_uncorrected_path = rl_uncorrected_file$file,
             file_read_lengths_uncorrected_size = rl_uncorrected_file$size,
             file_read_lengths_corrected_path = rl_corrected_file$file,
             file_read_lengths_corrected_size = rl_corrected_file$size,
...
for (bucket in buckets) {
    for (sample_name in samples_tbl$sample) {
        if (grepl(sample_name, bucket) || grepl(gsub("NA19", "NA129", sample_name), bucket)) {
            samples_tbl[which(samples_tbl$sample == sample_name), "bucket"] = bucket
            samples_tbl[which(samples_tbl$sample == sample_name), "run_type"] = ifelse(grepl("_CLR", bucket), "CLR", "CCS")
        }
    }
}
```
...
```{r eap_progress, warning=FALSE}
eap_stats = some_stats %>%
                filter(!grepl("SmallTest|Ecoli|Verily|Cell", sample_name)) %>%
                filter(cromwell_hash != "32f5116c-1ca4-40a0-bf44-1d0070e34957") %>%
                mutate(sample_name = ifelse(grepl("chip_", sample_name), "HCC1954T", sample_name)) %>%
                arrange(sample_name)

eap_stats$color = rainbow(nrow(eap_stats))
eap_stats$lty = round(runif(nrow(eap_stats), min = 1, max = 7))
eap_stats$pch = round(runif(nrow(eap_stats), min = 0, max = 25))

sl.indices = c(13,14,11,12,4,5,6,1,2,3,32,33,34,15,16,17,18,19,20,21,22,31,29,30,8,9,10,7,27,28,26,23)
eap_stats$order = NA
eap_stats[sl.indices,]$order = 1:length(sl.indices)

eap_stats$yield = 0
eap_stats$num_reads = 0
eap_stats$num_mapped_reads = 0
eap_stats$active_zmws = 0

eap_stats$read_length_min = 0
eap_stats$read_length_1stqu = 0
eap_stats$read_length_median = 0
eap_stats$read_length_mean = 0
eap_stats$read_length_sd = 0
eap_stats$read_length_3rdqu = 0
eap_stats$read_length_max = 0

eap_stats$zmw_min = 0
eap_stats$zmw_1stqu = 0
eap_stats$zmw_median = 0
eap_stats$zmw_mean = 0
eap_stats$zmw_sd = 0
eap_stats$zmw_3rdqu = 0
eap_stats$zmw_max = 0
...
rlu_files = foreach (i=1:nrow(eap_stats)) %dopar% {
    rlu_file = gs_get(eap_stats$file_read_lengths_uncorrected_path[i])
...
```{r read_length_load}
for (i in 1:nrow(eap_stats)) {
    print(eap_stats$sample_name[i])
    
    if (eap_stats$read_length_median[i] == 0) {
        rlu = read_delim(rlu_files[[i]], delim=' ', col_names=c("zmw", "range", "mq", "length"), progress=TRUE)
        
        rlu.g = rlu %>% group_by(zmw) %>% count()
        rlu.z = rlu %>% arrange(zmw) %>% select(zmw, length) %>% group_by(zmw) %>% summarize(n = n(), mean_length=mean(length))
        
        summary.rlu = summary(rlu$length)
        summary.rlu.gn = summary(rlu.g$n)
        
        eap_stats$yield[i] = sum(rlu$length)
        eap_stats$num_reads[i] = nrow(rlu)
        eap_stats$num_mapped_reads[i] = (rlu %>% filter(mq > 0) %>% nrow())
        eap_stats$active_zmws[i] = rlu.g %>% nrow()
        
        eap_stats$read_length_min[i] = summary.rlu[[1]]
        eap_stats$read_length_1stqu[i] = summary.rlu[[2]]
        eap_stats$read_length_median[i] = summary.rlu[[3]]
        eap_stats$read_length_mean[i] = summary.rlu[[4]]
        eap_stats$read_length_sd[i] = sd(rlu$length)
        eap_stats$read_length_3rdqu[i] = summary.rlu[[5]]
        eap_stats$read_length_max[i] = summary.rlu[[6]]
        
        eap_stats$zmw_min[i] = summary.rlu.gn[[1]]
        eap_stats$zmw_1stqu[i] = summary.rlu.gn[[2]]
        eap_stats$zmw_median[i] = summary.rlu.gn[[3]]
        eap_stats$zmw_mean[i] = summary.rlu.gn[[4]]
        eap_stats$zmw_sd[i] = sd(rlu.g$n)
        eap_stats$zmw_3rdqu[i] = summary.rlu.gn[[5]]
        eap_stats$zmw_max[i] = summary.rlu.gn[[6]]
        
        rl_h = hist(rlu$length, breaks=seq(0, as.integer(floor(1.1*max(rlu$length))), by=1), plot=FALSE)
        zmw_h = hist(rlu.g$n, breaks=seq(0, max(rlu.g$n)+10, by=1), plot=FALSE)
        
        rl_h_list[[i]] = rl_h$counts
        zmw_h_list[[i]] = zmw_h$counts
    }
}
```

```{r read_length_plots1}
par(mar=c(4, 7, 2, 2))

plot(0, 0, type="n", xlim=c(0, 20000), ylim=c(0, 2.5*max(unlist(lapply(rl_h_list, max)))), bty="n", las=1, ylab="", cex.axis=1.3, cex.lab=1.3, xlab="Read length (bp)")
mtext("Number of reads", 2, line=5, cex=1.3)

#eap_stats$read_length_median_v2 = 0

for (i in 1:nrow(eap_stats)) {
    if (!is.na(eap_stats$order[i])) {
        mids = seq(0.5, length(rl_h_list[[i]]), by=1)
        points(mids, rl_h_list[[i]], type="l", col=eap_stats$color[i], lty=eap_stats$lty[i])
        
        x = which.max(rl_h_list[[i]])
        y = 30000 + (eap_stats$order[i]*1750)
        text(x, y, labels=eap_stats$sample_name[i], col=eap_stats$color[i], cex=0.5, adj=0)
        segments(x, rl_h_list[[i]][x], x, y, col=gsub("FF$", "99", eap_stats$color[i]), lty=2)
        
        #eap_stats$read_length_median_v2[i] = rep(mids, rl_h_list[[i]]) %>% median()
    }
}
```

```{r read_length_plots2}
par(mar=c(5, 7, 2, 2))
layout.matrix = matrix(c(1, 2, 1, 2), nrow=2, ncol=2)
layout(mat = layout.matrix, heights=c(4, 2), widths=c(4, 4))
plot(0, 0, type="n", xlim=c(0, 50), ylim=c(1, max(unlist(lapply(zmw_h_list, max)))), bty="n", las=1, ylab="", xlab="", cex.axis=1.3, log="y")
mtext("Number of subreads", 2, line=5)
for (i in 1:nrow(eap_stats)) {
    mids = seq(0.5, length(zmw_h_list[[i]]), by=1)
    points(mids, zmw_h_list[[i]], type="l", col=eap_stats$color[i], lty=eap_stats$lty[i])
    
    if (eap_stats$sample_name[i] == "GWD_HG02982_CLR") {
        text(30, 50, eap_stats$sample_name[i], pos=2, cex=0.5)
    }
}
par(mar=c(5, 7, 0, 2))
boxplot(eap_stats$zmw_mean, horizontal = TRUE, ylim=c(0, 50), xlab="Number of passes", cex.axis=1.3, cex.lab=1.3)
```

```{r yield}
layout.matrix = matrix(c(1, 1, 2, 2), nrow=2, ncol=2)
layout(mat = layout.matrix, heights=c(4, 4), widths=c(3, 1))

par(mar=c(9, 6, 2, 1))
barplot(eap_stats$yield[sl.indices]/1e9, names.arg = eap_stats$sample_name[sl.indices], las=2, cex.names=0.5, ylim=c(0, max(eap_stats$yield))/1e9, col=c("#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#F28379", "#05C7F2","#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#D94436", "#05C7F2", "#D94436", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#D94436", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#05C7F2", "#D94436"))
abline(h=median(eap_stats$yield/1e9), lty=2, col="red")
mtext("Number of bases (Gbp)", 2, line=4)

par(mar=c(9, 3, 2, 2))
boxplot(eap_stats$yield/1e9, ylim=c(0, max(eap_stats$yield)/1e9), las=2)
```

```{r error_rate_by_passes_load}
er = read_delim("../../test.txt", delim=' ', col_names=c("np", "mismatches", "insertions", "deletions", "total"))
er2 = read_delim("../../test.unfiltered.txt", delim=' ', col_names=c("np", "mismatches", "insertions", "deletions", "total"))
er3 = read_delim("../../test.chr1.small_indels_suppressed.txt", delim=' ', col_names=c("np", "mismatches", "insertions", "deletions", "total"))
...
plot(era$np, -10*log10(cumsum(era$mismatches)/cumsum(era$total)), type="l", lwd=2, col="red", xlim=c(0, 40), ylim=c(0, 45), bty="n", ylab="Error rate (Phred-scale)", xlab="Cumulative number of passes", las=1, cex.axis=1.3, cex.lab=1.3)
points(era$np, -10*log10(cumsum(era$insertions)/cumsum(era$total)), type="l", lwd=2, col="green")
points(era$np, -10*log10(cumsum(era$deletions)/cumsum(era$total)), type="l", lwd=2, col="blue")
points(era$np, -10*log10(cumsum(era$mismatches + era$insertions + era$deletions)/cumsum(era$mismatches + era$insertions + era$deletions + era$total)), type="l", lwd=2, col="black")

points(er3$np, -10*log10(cumsum(er3$insertions)/cumsum(er3$total)), type="l", lwd=1, col="green")
points(er3$np, -10*log10(cumsum(er3$deletions)/cumsum(er3$total)), type="l", lwd=1, col="blue")
points(er3$np, -10*log10(cumsum(er3$mismatches + er3$insertions + er3$deletions)/cumsum(er3$mismatches + er3$insertions + er3$deletions + er3$total)), type="l", lwd=1, col="black")

legend("bottomright", c("Mismatches", "Insertions", "Deletions", "All"), col=c("red", "green", "blue", "black"), lwd=2)
points(1, -10*log10(er2$mismatches/er2$total), pch=4, col="red")
points(1, -10*log10(er2$insertions/er2$total), pch=4, col="green")
points(1, -10*log10(er2$deletions/er2$total), pch=4, col="blue")

abline(h=-10*log10(13493671/3123730626), lwd=1, lty=2, col="red")
abline(h=-10*log10(860354/3123730626), lwd=1, lty=2, col="green")
abline(h=-10*log10(629002/3123730626), lwd=1, lty=2, col="blue")
abline(h=-10*log10((13493671+860354+629002)/3123730626), lwd=1, lty=2, col="black")
```

```{r error_hist}
di = read_delim("../../test.chr20.small_indels_suppressed.insHist.txt", delim=' ', col_names=c("left", "right"))
dd = read_delim("../../test.chr20.small_indels_suppressed.delHist.txt", delim=' ', col_names=c("left", "right"))
```

```{r ccs}
ccsm = rbind(
    eap_stats[sl.indices,]$`success_(without_retry)_ccs_generated`,
    eap_stats[sl.indices,]$`success_(with_retry)_ccs_generated`,
    eap_stats[sl.indices,]$failed_below_snr_threshold,
    eap_stats[sl.indices,]$failed_no_usable_subreads,
    eap_stats[sl.indices,]$failed_insert_size_too_long,
    eap_stats[sl.indices,]$failed_insert_size_too_small,
    eap_stats[sl.indices,]$failed_not_enough_full_passes,
    eap_stats[sl.indices,]$failed_too_many_unusable_subreads,
    eap_stats[sl.indices,]$failed_ccs_did_not_converge,
    eap_stats[sl.indices,]$failed_ccs_below_minimum_predicted_accuracy,
    eap_stats[sl.indices,]$failed_unknown_error_during_processing
)

layout.matrix = matrix(c(1, 1, 2, 2), nrow=2, ncol=2)
layout(mat = layout.matrix, heights=c(4, 4), widths=c(2, 2))

par(mar=c(8, 6, 2, 0))
colors = c(cm.colors(2), heat.colors(9))
barplot(ccsm, col=colors, las=2, names.arg = eap_stats[sl.indices,]$sample_name, cex.names = 0.5, cex.axis=1.3, ylab="")
mtext("# reads (corrected + discarded)", side=2, line=5)
...
c(
    ...
    "Success (with retry)",
    "Failed (below snr threshold)",
    "Failed (no usable subreads)",
    "Failed (insert size too long)",
    "Failed (insert size too small)",
    "Failed (not enough full passes)",
    "Failed (too many unusable subreads)",
    "Failed (ccs did not converge)",
    "Failed (ccs below minimum predicted accuracy)",
    "Failed (unknown error during processing)"     
), fill=colors, bty="n")

uncorrected_pct_mean = mean(100*(colSums(ccsm[c(3:11),]) / eap_stats[sl.indices,]$TOTAL_READS_UNCORRECTED)[c(-5)])
uncorrected_pct_sd = sd(100*(colSums(ccsm[c(3:11),]) / eap_stats[sl.indices,]$TOTAL_READS_UNCORRECTED)[c(-5)])

```

```{r yield}
plot(eap_stats$active_zmws, eap_stats$yield/1e9, bty="n", las=1, xlab="Active ZMWs", ylab="Yield (Gbp)", pch=19)
abline(lm(eap_stats$yield/1e9 ~ eap_stats$active_zmws), col="red")
text(2e6, 400, paste("R^2 =", sprintf("%.2f", summary(lm(eap_stats$yield/1e9 ~ eap_stats$active_zmws))$r.squared)))
plot(eap_stats$read_length_median, eap_stats$yield/1e9, bty="n", las=1, xlab="Median read length", ylab="Yield (Gbp)", pch=19)
abline(lm(eap_stats$yield/1e9 ~ eap_stats$read_length_median), col="red")
text(10500, 320, paste("R^2 =", sprintf("%.2f", summary(lm(eap_stats$yield/1e9 ~ eap_stats$read_length_median))$r.squared)))
```
