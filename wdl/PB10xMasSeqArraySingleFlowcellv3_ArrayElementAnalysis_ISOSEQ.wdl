...
workflow PB10xMasSeqArraySingleFlowcellv3_ArrayElementAnalysis {
    ...
    call Utils.MergeBams as t_09_MergeAlignedCCSArrayElements {
        input:
...
#               1000?
    ...
    call Utils.FilterMasSeqReadsWithGatk as t_11_AlignmentFilterForCcsArrayElements {
        input:
            bam_file = t_09_MergeAlignedCCSArrayElements.merged_bam,
            bam_index = t_09_MergeAlignedCCSArrayElements.merged_bai,
    ...
    call Utils.MergeBams as t_12_MergeAllAlignedAndFilteredArrayElements {
        input:
            bams = [t_11_AlignmentFilterForCcsArrayElements.bam],
            prefix = SM + "_all_array_elements_aligned_for_txome_discovery",
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    call StringTie2.Quantify as t_13_ST2_Quant {
        input:
            aligned_bam = t_12_MergeAllAlignedAndFilteredArrayElements.merged_bam,
            aligned_bai = t_12_MergeAllAlignedAndFilteredArrayElements.merged_bai,
    ...
    call StringTie2.ExtractTranscriptSequences as t_14_ST2_ExtractTranscriptSequences  {
        input:
            ref_fasta = ref_fasta,
            ref_fasta_fai = ref_fasta_index,
            gtf = t_13_ST2_Quant.st_gtf,
            prefix = SM + "_StringTie2_ExtractTranscriptSequences",
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    call StringTie2.CompareTranscriptomes as t_15_ST2_CompareTranscriptomes {
        input:
            guide_gtf = genome_annotation_gtf,
            new_gtf = t_13_ST2_Quant.st_gtf,
    ...
    # CCS
    call Utils.ShardReads as t_16_ShardS2ECcsArrayElements {
        input:
    ...
    scatter (ccsi in range(length(t_16_ShardS2ECcsArrayElements.shards))) {
        File ccs_array_element_shard = t_16_ShardS2ECcsArrayElements.shards[ccsi]

        call Utils.IndexBam as t_17_IndexCcsArrayElementShard {
            input:
        ...
        call LONGBOW.Pad as t_18_LongbowPadCCSArrayElementUMIs {
            input:
        ...
        call LONGBOW.Pad as t_19_LongbowPadCCSArrayElementCBCs {
            input:
                reads = t_18_LongbowPadCCSArrayElementUMIs.padded_tag_bam,
                model = mas_seq_model,
                tag_to_expand = "CR",
        ...
        call LONGBOW.Correct as t_20_LongbowCorrectCCSCorrectedArrayElementCBCs {
            input:
                reads = t_19_LongbowPadCCSArrayElementCBCs.padded_tag_bam,
        ...
        call TENX.AdjustUmiSequenceWithAdapterAlignment as t_21_AdjustCCSUMIs {
            input:
                bam = t_20_LongbowCorrectCCSCorrectedArrayElementCBCs.corrected_barcodes_bam,
    ...
    call Utils.MergeBams as t_22_MergeLongbowPaddedCBCCorrectedCCSArrayElements {
        input:
            bams = t_21_AdjustCCSUMIs.output_bam,
            prefix = SM + "_ccs_array_elements_aligned_annotated_padded_CBC_corrected",
            runtime_attr_override = disable_preemption_runtime_attrs
    }
    call Utils.MergeBams as t_23_MergeLongbowPaddedCBCUncorrectableCCSArrayElements {
        input:
            bams = t_20_LongbowCorrectCCSCorrectedArrayElementCBCs.uncorrected_barcodes_bam,
    ...
    call TX_PRE.RestoreOriginalReadNames as t_24_RestoreCcsOriginalReadNames {
        input:
            bam = t_22_MergeLongbowPaddedCBCCorrectedCCSArrayElements.merged_bam,
            prefix =  SM + "_CCS_cbc_annotated_array_elements_padded_original_names",
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    # Merge Aligned CCS and Reclaimed reads together:
    call Utils.MergeBams as t_25_MergeAllAnnotatedArrayElementsWithOriginalNames {
        input:
            bams = [t_24_RestoreCcsOriginalReadNames.bam_out],
    ...
    call TX_PRE.GffCompare as t_26_GffCompareStringtie2toGencode {
        input:
            gff_ref = t_13_ST2_Quant.st_gtf,
            gff_query = genome_annotation_gtf,
            ref_fasta = ref_fasta,
            ref_fasta_index = ref_fasta_index,
            runtime_attr_override = disable_preemption_runtime_attrs
    }
    call TX_PRE.GffCompare as t_27_GffCompareGencodetoStringtie2 {
        input:
            gff_ref = genome_annotation_gtf,
            gff_query = t_13_ST2_Quant.st_gtf,
            ref_fasta = ref_fasta,
            ref_fasta_index = ref_fasta_index,
            runtime_attr_override = disable_preemption_runtime_attrs
    }
    ...
    # Split by contig:
    call TX_PRE.SplitBamByContig as t_28_SplitArrayElementsByContig {
        input:
            bam = t_25_MergeAllAnnotatedArrayElementsWithOriginalNames.merged_bam,
            prefix = SM + "_all_cbc_annotated_array_elements_padded_original_names",
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    # For each contig:
    scatter (i in range(length(t_28_SplitArrayElementsByContig.contig_bams))) {

        File contig_bam = t_28_SplitArrayElementsByContig.contig_bams[i]
        String contig_name = t_28_SplitArrayElementsByContig.contig_names[i]

        # Create a GFF file:
        call TX_PRE.ConvertSplicedBamToGff as t_29_ConvertSplicedBamToGff {
            input:
                bam = contig_bam,
                runtime_attr_override = disable_preemption_runtime_attrs
        }
        ...
        # Compare GFF files:
        call TX_PRE.GffCompare as t_30_GffCompareStringtie2toMasSeqReads {
            input:
                gff_ref = t_13_ST2_Quant.st_gtf,
                gff_query = t_29_ConvertSplicedBamToGff.gff,
                ref_fasta = ref_fasta,
                ref_fasta_index = ref_fasta_index,
                runtime_attr_override = disable_preemption_runtime_attrs
        }

        call TX_PRE.GffCompare as t_31_GffCompareGencodetoMasSeqReads {
            input:
                gff_ref = genome_annotation_gtf,
                gff_query = t_29_ConvertSplicedBamToGff.gff,
                ref_fasta = ref_fasta,
        ...
        call TX_POST.QuantifyGffComparison as t_32_QuantifyGffComparison {
            input:
                genome_gtf = genome_annotation_gtf,
                st2_gencode_refmap = t_26_GffCompareStringtie2toGencode.refmap,
                st2_gencode_tmap = t_26_GffCompareStringtie2toGencode.tmap,
                st2_read_refmap = t_30_GffCompareStringtie2toMasSeqReads.refmap,
                st2_read_tmap = t_30_GffCompareStringtie2toMasSeqReads.tmap,
                gencode_st2_refmap = t_27_GffCompareGencodetoStringtie2.refmap,
                gencode_st2_tmap = t_27_GffCompareGencodetoStringtie2.tmap,
                gencode_read_refmap = t_31_GffCompareGencodetoMasSeqReads.refmap,
                gencode_read_tmap = t_31_GffCompareGencodetoMasSeqReads.tmap,
    ...
    call TX_POST.CombineEqClassFiles as t_33_CombineEqClassFiles {
        input:
            gene_eq_class_definitions = t_32_QuantifyGffComparison.gene_eq_class_labels_file,
            gene_assignment_files = t_32_QuantifyGffComparison.gene_assignments_file,
            equivalence_class_definitions = t_32_QuantifyGffComparison.tx_equivalence_class_labels_file,
            equivalence_classes = t_32_QuantifyGffComparison.tx_equivalence_class_file,
    ...
    call TX_POST.CopyEqClassInfoToTag as t_34_CopyEqClassInfoToTag {
        input:
            bam = t_25_MergeAllAnnotatedArrayElementsWithOriginalNames.merged_bam,
            eq_class_file = t_33_CombineEqClassFiles.combined_tx_eq_class_assignments,
            prefix = SM + "_annotated_array_elements_for_quant_with_gene_names",
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    call TX_PRE.CorrectUmisWithSetCover as t_35_CorrectUmisWithSetCover {
        input:
            bam = t_34_CopyEqClassInfoToTag.bam_out,
            prefix = SM + "_annotated_array_elements_for_quant_with_gene_names",
            runtime_attr_override = disable_preemption_runtime_attrs
    }
    ...
    call Utils.Bamtools as t_36_GetCcsCorrectedReadsWithCorrectedUmis {
        input:
            bamfile = t_35_CorrectUmisWithSetCover.corrected_umi_reads,
    ...
    call UMI_TOOLS.Run_Group as t_37_UMIToolsGroup {
        input:
            aligned_transcriptome_reads = t_35_CorrectUmisWithSetCover.corrected_umi_reads,
            aligned_transcriptome_reads_index = t_35_CorrectUmisWithSetCover.corrected_umi_reads_index,
            do_per_cell = true,
    ...
    call TX_POST.CreateCountMatrixFromAnnotatedBam as t_38_CreateCCSCountMatrixFromAnnotatedBam {
        input:
            annotated_transcriptome_bam = t_36_GetCcsCorrectedReadsWithCorrectedUmis.bam_out,
            tx_equivalence_class_assignments = t_33_CombineEqClassFiles.combined_tx_eq_class_assignments,
            umi_tag = "BX",
            prefix = SM + "_ccs_gene_tx_expression_count_matrix",
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    call TX_POST.CreateCountMatrixAnndataFromEquivalenceClasses as t_39_CreateCCSCountMatrixAnndataFromEqClasses {
        input:
            count_matrix_tsv = t_38_CreateCCSCountMatrixFromAnnotatedBam.count_matrix,
            genome_annotation_gtf_file = t_13_ST2_Quant.st_gtf,
            ...
            tx_equivalence_class_assignments = t_33_CombineEqClassFiles.combined_tx_eq_class_assignments,
            tx_equivalence_class_definitions = t_33_CombineEqClassFiles.combined_tx_eq_class_defs,
            gene_equivalence_class_assignments = t_33_CombineEqClassFiles.combined_gene_eq_class_assignments,
            gene_equivalence_class_definitions = t_33_CombineEqClassFiles.combined_gene_eq_class_defs,
            prefix = SM + "_ccs_gene_tx_expression_count_matrix",
    ...
    call TX_POST.CreateCountMatrixFromAnnotatedBam as t_40_CreateOverallCountMatrixFromAnnotatedBam {
        input:
            annotated_transcriptome_bam = t_35_CorrectUmisWithSetCover.corrected_umi_reads,
            tx_equivalence_class_assignments = t_33_CombineEqClassFiles.combined_tx_eq_class_assignments,
            umi_tag = "BX",
    ...
    call TX_POST.CreateCountMatrixAnndataFromEquivalenceClasses as t_41_CreateOverallCountMatrixAnndataFromEqClasses {
        input:
            count_matrix_tsv = t_40_CreateOverallCountMatrixFromAnnotatedBam.count_matrix,
            genome_annotation_gtf_file = t_13_ST2_Quant.st_gtf,
            gencode_reference_gtf_file = genome_annotation_gtf,
            overlap_intervals = intervals_of_interest,
            overlap_interval_label = interval_overlap_name,
    ...
    call AM.SamtoolsStats as t_42_AlignedArrayElementStats {
        input:
            bam = t_10_MergeAllAlignedArrayElementsNonTruncated.merged_bam,
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    call AM.SamtoolsStats as t_43_AlignedFilteredArrayElementStats {
        input:
            bam = t_12_MergeAllAlignedAndFilteredArrayElements.merged_bam,
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    call AM.SamtoolsStats as t_44_AlignedAnnotatedArrayElementsForQuantStats {
        input:
            bam = t_35_CorrectUmisWithSetCover.corrected_umi_reads,
            runtime_attr_override = disable_preemption_runtime_attrs
    }

    call LONGBOW.AggregateCorrectLogStats as t_45_AggregateLongbowCorrectStats {
        input:
            longbow_correct_log_files = t_20_LongbowCorrectCCSCorrectedArrayElementCBCs.log,
...
#    File keyfile = t_41_CreateOverallCountMatrixAnndataFromEqClasses.transcript_gene_count_anndata_h5ad

    # This seems to take longer to get to:
    File keyfile = t_37_UMIToolsGroup.output_tsv
    ...
    call FF.FinalizeToDir as t_46_FinalizeEqClasses {
        input:
            files = [
                t_33_CombineEqClassFiles.combined_gene_eq_class_defs,
                t_33_CombineEqClassFiles.combined_gene_eq_class_assignments,
                t_33_CombineEqClassFiles.combined_tx_eq_class_defs,
                t_33_CombineEqClassFiles.combined_tx_eq_class_assignments,
            ],
            outdir = quant_dir + "/eqivalence_classes",
            keyfile = keyfile
    }

    call FF.FinalizeToDir as t_47_FinalizeUmiToolsOutputs {
        input:
            files = [
                t_37_UMIToolsGroup.output_bam,
                t_37_UMIToolsGroup.output_tsv,
            ],
    ...
    # CCS:
    call FF.FinalizeToDir as t_48_FinalizeCCSTxAndGeneAssignments {
        input:
            files = [
                t_38_CreateCCSCountMatrixFromAnnotatedBam.count_matrix,
                t_39_CreateCCSCountMatrixAnndataFromEqClasses.transcript_gene_count_anndata_h5ad,
            ],
            outdir = quant_dir + "/CCS",
            keyfile = keyfile
    }

    call FF.FinalizeToDir as t_49_FinalizeCCSRawQuantPickles {
        input:
            files = t_39_CreateCCSCountMatrixAnndataFromEqClasses.pickles,
            outdir = quant_dir + "/CCS",
            keyfile = keyfile
    }

    # Overall:
    call FF.FinalizeToDir as t_50_FinalizeOverallTxAndGeneAssignments {
        input:
            files = [
                t_40_CreateOverallCountMatrixFromAnnotatedBam.count_matrix,
                t_41_CreateOverallCountMatrixAnndataFromEqClasses.transcript_gene_count_anndata_h5ad,
            ],
            outdir = quant_dir + "/Overall",
            keyfile = keyfile
    }

    call FF.FinalizeToDir as t_51_FinalizeOverallRawQuantPickles {
        input:
            files = t_41_CreateOverallCountMatrixAnndataFromEqClasses.pickles,
            outdir = quant_dir + "/Overall",
            keyfile = keyfile
    }

    call FF.FinalizeToDir as t_52_FinalizeRefAndSt2Comparisons {
        input:
            files = [
                t_26_GffCompareStringtie2toGencode.refmap,
                t_26_GffCompareStringtie2toGencode.tmap,
                t_26_GffCompareStringtie2toGencode.tracking,
                t_26_GffCompareStringtie2toGencode.loci,
                t_26_GffCompareStringtie2toGencode.annotated_gtf,
                t_26_GffCompareStringtie2toGencode.stats,
                t_26_GffCompareStringtie2toGencode.log,

                t_27_GffCompareGencodetoStringtie2.refmap,
                t_27_GffCompareGencodetoStringtie2.tmap,
                t_27_GffCompareGencodetoStringtie2.tracking,
                t_27_GffCompareGencodetoStringtie2.loci,
                t_27_GffCompareGencodetoStringtie2.annotated_gtf,
                t_27_GffCompareGencodetoStringtie2.stats,
                t_27_GffCompareGencodetoStringtie2.log,
            ],
    ...
    scatter (i in range(length(t_28_SplitArrayElementsByContig.contig_bams))) {
        String contig = t_28_SplitArrayElementsByContig.contig_names[i]

        call FF.FinalizeToDir as t_53_FinalizeTxAndGeneAssignmentsByContig {
            input:
                files = [
                    t_30_GffCompareStringtie2toMasSeqReads.refmap[i],
                    t_30_GffCompareStringtie2toMasSeqReads.tmap[i],
                    t_30_GffCompareStringtie2toMasSeqReads.tracking[i],
                    t_30_GffCompareStringtie2toMasSeqReads.loci[i],
                    t_30_GffCompareStringtie2toMasSeqReads.annotated_gtf[i],
                    t_30_GffCompareStringtie2toMasSeqReads.stats[i],
                    t_30_GffCompareStringtie2toMasSeqReads.log[i],

                    t_31_GffCompareGencodetoMasSeqReads.refmap[i],
                    t_31_GffCompareGencodetoMasSeqReads.tmap[i],
                    t_31_GffCompareGencodetoMasSeqReads.tracking[i],
                    t_31_GffCompareGencodetoMasSeqReads.loci[i],
                    t_31_GffCompareGencodetoMasSeqReads.annotated_gtf[i],
                    t_31_GffCompareGencodetoMasSeqReads.stats[i],
                    t_31_GffCompareGencodetoMasSeqReads.log[i],

                    t_32_QuantifyGffComparison.gene_assignments_file[i],
                    t_32_QuantifyGffComparison.tx_equivalence_class_labels_file[i],
                    t_32_QuantifyGffComparison.tx_equivalence_class_file[i],
                    t_32_QuantifyGffComparison.graph_gpickle[i],
                ],
    ...
    call FF.FinalizeToDir as t_54_FinalizeIntermediateAnnotatedArrayElements {
        input:
            files = [
                t_10_MergeAllAlignedArrayElementsNonTruncated.merged_bam,
                t_10_MergeAllAlignedArrayElementsNonTruncated.merged_bai,

                t_12_MergeAllAlignedAndFilteredArrayElements.merged_bam,
                t_12_MergeAllAlignedAndFilteredArrayElements.merged_bai,

                t_22_MergeLongbowPaddedCBCCorrectedCCSArrayElements.merged_bam,
                t_22_MergeLongbowPaddedCBCCorrectedCCSArrayElements.merged_bai,
                t_23_MergeLongbowPaddedCBCUncorrectableCCSArrayElements.merged_bam,
                t_23_MergeLongbowPaddedCBCUncorrectableCCSArrayElements.merged_bai,

                t_24_RestoreCcsOriginalReadNames.bam_out,

                t_25_MergeAllAnnotatedArrayElementsWithOriginalNames.merged_bam,
                t_25_MergeAllAnnotatedArrayElementsWithOriginalNames.merged_bai,

                t_35_CorrectUmisWithSetCover.uncorrected_umi_reads
            ],
            outdir = intermediate_array_elements_dir,
            keyfile = keyfile
    }

    call FF.FinalizeToDir as t_55_FinalizeAnnotatedArrayElements {
        input:
            files = [
                t_35_CorrectUmisWithSetCover.corrected_umi_reads,
                t_35_CorrectUmisWithSetCover.corrected_umi_reads_index,

                t_36_GetCcsCorrectedReadsWithCorrectedUmis.bam_out,
    ...
    ##############################################################################################################
    # Finalize meta files:
    call FF.FinalizeToDir as t_56_FinalizeMeta {
        input:
            files = [
    ...
    call FF.FinalizeToDir as t_57_FinalizeCCSCBCcorrectionLogsToMeta {
        input:
            files = t_20_LongbowCorrectCCSCorrectedArrayElementCBCs.log,
            outdir = meta_files_dir + "/" + "ccs_cbc_correction_logs",
            keyfile = keyfile
    }

    call FF.FinalizeToDir as t_58_FinalizeCCSUmiAdjustmentLogs {
        input:
            files = t_21_AdjustCCSUMIs.log,
    ...
    if ( !is_SIRV_data ) {
        call FF.FinalizeToDir as t_59_FinalizeDiscoveredTranscriptome {
            input:
                files = [
                    t_13_ST2_Quant.st_gtf,
                    t_14_ST2_ExtractTranscriptSequences.transcripts_fa,
                    t_14_ST2_ExtractTranscriptSequences.transcripts_fai,
                    t_14_ST2_ExtractTranscriptSequences.transcripts_dict,
                    t_15_ST2_CompareTranscriptomes.annotated_gtf,
                    t_15_ST2_CompareTranscriptomes.loci,
                    t_15_ST2_CompareTranscriptomes.stats,
                    t_15_ST2_CompareTranscriptomes.tracking,
                    t_15_ST2_CompareTranscriptomes.refmap,
                    t_15_ST2_CompareTranscriptomes.tmap,
                ],
    ...
    call FF.FinalizeToDir as t_60_FinalizeQuantArrayElementStats {
        input:
            files = [
                t_44_AlignedAnnotatedArrayElementsForQuantStats.raw_stats,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.summary_stats,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.first_frag_qual,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.last_frag_qual,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.first_frag_gc_content,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.last_frag_gc_content,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.acgt_content_per_cycle,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.insert_size,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.read_length_dist,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.indel_distribution,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.indels_per_cycle,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.coverage_distribution,
                t_44_AlignedAnnotatedArrayElementsForQuantStats.gc_depth,
            ],
            outdir = stats_out_dir + "/array_elements_for_quant/",
            keyfile = keyfile
    }

    call FF.FinalizeToDir as t_61_FinalizeTxomeDiscoveryArrayElementStats {
        input:
            files = [
                t_43_AlignedFilteredArrayElementStats.raw_stats,
                t_43_AlignedFilteredArrayElementStats.summary_stats,
                t_43_AlignedFilteredArrayElementStats.first_frag_qual,
                t_43_AlignedFilteredArrayElementStats.last_frag_qual,
                t_43_AlignedFilteredArrayElementStats.first_frag_gc_content,
                t_43_AlignedFilteredArrayElementStats.last_frag_gc_content,
                t_43_AlignedFilteredArrayElementStats.acgt_content_per_cycle,
                t_43_AlignedFilteredArrayElementStats.insert_size,
                t_43_AlignedFilteredArrayElementStats.read_length_dist,
                t_43_AlignedFilteredArrayElementStats.indel_distribution,
                t_43_AlignedFilteredArrayElementStats.indels_per_cycle,
                t_43_AlignedFilteredArrayElementStats.coverage_distribution,
                t_43_AlignedFilteredArrayElementStats.gc_depth,
            ],
            outdir = stats_out_dir + "/array_elements_for_transcriptome_discovery/",
            keyfile = keyfile
    }

    call FF.FinalizeToDir as t_62_FinalizeAlignedArrayElementStats {
        input:
            files = [
                t_42_AlignedArrayElementStats.raw_stats,
                t_42_AlignedArrayElementStats.summary_stats,
                t_42_AlignedArrayElementStats.first_frag_qual,
                t_42_AlignedArrayElementStats.last_frag_qual,
                t_42_AlignedArrayElementStats.first_frag_gc_content,
                t_42_AlignedArrayElementStats.last_frag_gc_content,
                t_42_AlignedArrayElementStats.acgt_content_per_cycle,
                t_42_AlignedArrayElementStats.insert_size,
                t_42_AlignedArrayElementStats.read_length_dist,
                t_42_AlignedArrayElementStats.indel_distribution,
                t_42_AlignedArrayElementStats.indels_per_cycle,
                t_42_AlignedArrayElementStats.coverage_distribution,
                t_42_AlignedArrayElementStats.gc_depth,
            ],
    ...
    call FF.WriteCompletionFile as t_63_WriteCompletionFile {
        input:
            outdir = base_out_dir + "/",
            keyfile = keyfile
    }
