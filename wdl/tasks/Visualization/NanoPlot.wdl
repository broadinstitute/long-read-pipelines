...
# # example output summary from nanoplot
...
task NanoPlotFromFastqs {
    ...
    parameter_meta {
        fastqs: {
            desciption: "A list of fastq files to use as input",
            localization_optional: true
        }
        is_ont_rich_fastq: "If the input FASTQs are from ONT generated by albacore, MinKNOW or with additional information concerning channel and time"
        runtime_attr_override: "Override the default runtime attributes"
    }
    ...
    input {
        ...
        Boolean is_ont_rich_fastq

        String disk_type = "SSD"
        RuntimeAttr? runtime_attr_override
    }

    String input_fmt_choice = if (is_ont_rich_fastq) then "--fastq_rich" else "--fastq"
    ...
    #########################
    Int pd_disk_size = 50 + ceil(size(fastqs, "GiB"))
    Int local_disk_size = if(size(fastqs, "GiB")>300) then 750 else 375
