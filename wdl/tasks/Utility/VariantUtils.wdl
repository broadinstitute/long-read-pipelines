...
task CopyDP_MINToDP {
    ...
    # TODO: Generalize this task to work with any INFO field.
    meta {
        description: "Copy the DP_MIN field to the DP field in a VCF file (DP_MIN is generated by Clair3).  This enables joint call cohorts with Clair3 (and other callers using DP_MIN) and GATK-called GVCF files (in combination with a new config options to tell GLNexus to use the DP field for its analysis)."
    }
