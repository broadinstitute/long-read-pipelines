...
task ValidateSamFile {
    meta {
        description: "Validate BAM using Picard ValidateSamFile"
    }
    ...
    Int disk_size = ceil(size(bam, "GiB")) + 20
    ...
    RuntimeAttr default_attr = object {
        cpu_cores: 1,
        mem_gb: 4,  # Lower memory since short reads need less per-read processing
        disk_gb: disk_size,
        preemptible_tries: 3,
        max_retries: 1,
...
task SortBam {
    meta {
        description: "Sort BAM file by coordinate"
    }
    ...
    Int disk_size = ceil(3 * size(input_bam, "GiB")) + 20

    command <<<
        set -e
        
        # For short reads, we can use more aggressive memory settings
        gatk SortSam \
            --java-options "-Xmx3g" \
            -I ~{input_bam} \
            -O ~{output_basename}.sorted.bam \
    ...
    output {
        File sorted_bam = "~{output_basename}.sorted.bam"
        File sorted_bai = "~{output_basename}.sorted.bai"
    }
...
task MarkDuplicates {
    ...
    command <<<
        gatk MarkDuplicates \
            --java-options "-Xmx4g" \
            -I ~{input_bam} \
            -O ~{output_basename}.dedup.bam \
            -M ~{output_basename}.metrics.txt \
            --CREATE_INDEX true \
            --VALIDATION_STRINGENCY SILENT
    >>>

    output {
        File deduped_bam = "~{output_basename}.dedup.bam"
        File deduped_bai = "~{output_basename}.dedup.bai"
