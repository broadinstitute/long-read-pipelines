...
task ImportGVCFs {
    ...
    String genomicsdb_arg = if has_existing_genomicsdb_tar then "--genomicsdb-update-workspace-path ~{genomicsdb_name}" else "--genomicsdb-workspace-path ~{genomicsdb_name}"
    ...
    command <<<
        ...
        if [[ ~{has_existing_genomicsdb_tar} == "true" ]] ; then
            ...
            grep 'sample_name' "${GENOMICSDB_DIR}/callset.json" | awk '{print $NF}' | tr -d '", ' | sort > genomicsdb_samples.txt
            awk '{print $1}' ~{sample_name_map} | sort > input_samples.txt
        ...
        tar -cf ~{genomicsdb_name}.tar ~{genomicsdb_name}
    >>>

    output {
        File output_genomicsdb = "~{genomicsdb_name}.tar"
    }
