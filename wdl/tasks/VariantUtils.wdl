    ...
    command <<<
        ...
        function formatVCF() {
            ...
            while read VCF_FILE; do
                ...
                bcftools view -h ${VCF_FILE}_pass.vcf > ${VCF_FILE}_filtered.vcf
                ls -lah ~{work_dir}/regions.bed
                ~{docker_dir}/bedtools intersect -u -a ${VCF_FILE}_pass.vcf -b ~{work_dir}/regions.bed >> ${VCF_FILE}_filtered.vcf
...
task ConcatVCFs {
    ...
    output {
        ...
        File concat_tbi = "~{prefix}.vcf.gz.tbi"
    }
...
task SubsetVCFToSVs {
    meta {
        description: "Subset VCFs to structural variants (>= 50 bp events)"
    }

    input {
        File vcf

        String prefix = "sv_subset"
    ...
    command <<<
        ...
        bcftools view ~{vcf} | \
            grep -v 'SVTYPE=SNV' | \
            perl -lanE '/SVLEN=(\d+);/; print "$_" if abs($1) > 50 || /^#/' | \
            bgzip > ~{prefix}.vcf.gz
    ...
    output {
        File sv_vcf = "~{prefix}.vcf.gz"
        File sv_tbi = "~{prefix}.vcf.gz.tbi"
    }
...
task VCFToBed {
    ...
    command <<<
        ...
        bcftools view ~{vcf} | \
            grep -v '^#' | \
            perl -lanE 'print "@F[0]\t@F[1]\t$1" if /END=(\d+);/' \
            > ~{prefix}.bed
    >>>
