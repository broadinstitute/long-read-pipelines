...
task VariantFiltration {
    ...
    String a = basename(vcf, ".vcf.gz")
    String b = basename(vcf, ".g.vcf.bgz")
    String c = basename(vcf)
    String ext = if (a != c) then "vcf.gz" else "g.vcf.bgz"
    String base = basename(basename(vcf, ".vcf.gz"), ".g.vcf.bgz")
    String out = "~{base}.~{out_appendix}.~{ext}"
    ...
    #########################
    Int disk_size = 10 + 2*ceil(size(vcf, "GiB")) + ceil(size(vcf, "GiB"))
