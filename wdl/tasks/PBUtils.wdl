...
task GetRunInfo {
    ...
    String sm_arg = if defined(SM) then " --SM " else ""
    ...
    command <<<
        ...
        python /usr/local/bin/detect_run_info.py ~{gcs_dir} ~{sm_arg}~{default="" sep=" --SM " SM} ~{bam_suffix_arg}~{default="" sep=" --BS " bam_suffix} > run_info.txt
...
task Align {
    ...
    String drop_pulse_tags_option = if drop_per_base_N_pulse_tags then " --strip " else ""
    ...
    output {
        ...
        File aligned_pbi = "~{prefix}.bam.pbi"
    }
