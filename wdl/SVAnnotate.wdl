...
workflow SVAnnotate {
    ...
    call Annotate { input: vcf=Standardize.vcf_std, gtf=gtf, prefix=prefix }

    output {
        File vcf_std = Standardize.vcf_std
        File vcf_anno = Annotate.vcf_anno
    }
}
...
task Annotate {
    ...
    command <<<
        ...
        tabix ~{vcf}
        gatk SVAnnotate -V ~{vcf} --protein-coding-gtf ~{gtf} -O ~{prefix}.anno.vcf
