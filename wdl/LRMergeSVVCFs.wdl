...
workflow LRMergeSVVCFs {
    ...
    scatter (contig_name in GetContigNames.contig_names) {
        ...
        call LRRegenotype.LRRegenotype {
            input:
                merged_vcf_gz = SubsetVCF.subset_vcf,
    ...
    call VariantUtils.ConcatVCFs { input: vcfs = LRRegenotype.vcf_gz, tbis = LRRegenotype.tbi, prefix = prefix }
    ...
    # Finalize
    call FF.FinalizeToFile as FinalizeStandardizedVCF { input: outdir = outdir, file = ConcatVCFs.concat_vcf }
    call FF.FinalizeToFile as FinalizeStandardizedTBI { input: outdir = outdir, file = ConcatVCFs.concat_tbi }
