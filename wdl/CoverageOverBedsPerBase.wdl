...
workflow CoverageOverBedsPerBase {
    ...
    # call SplitCollectMerge {
    #     input:
    #         bam = bam, bai = bai, four_col_bed = four_col_bed,
    #         min_base_q = min_base_q, min_map_q = min_map_q,
    #         fail_on_non_4_col_bed = fail_on_non_4_col_bed
    # }

    # call FF.FinalizeToFile { input: outdir = dir, file = SplitCollectMerge.depth }
...
task PadAndSplit {
    ...
    command <<<
        ...
        awk 'BEGIN{OFS="\t"} {a=$2-100; b=$3+100; if(a<0) a=0; print $1, a, b, $4}' ~{four_col_bed} > padded.tsv
        if awk -F '\t' 'NF!=4{exit 1}' ~{four_col_bed} ; then
            cut -f 5- > trailing.tsv
            paste -d '\t' padded.tsv trailing.tsv > padded.bed
        else
            mv padded.tsv padded.bed
        fi

        # note --additional-suffix= is supported only after ≥ 8.16 of gnu split
        split -d -l ~{split_line_cnt} padded.bed --additional-suffix=".bed" splitted_padded_
    >>>

    output {
        File padded_bed = "padded.bed"
        Array[File] splitted_padded_beds = glob("splitted_padded_*.bed")
    }
    runtime {
        cpu:            1
        memory:         "4 GiB"
