...
workflow MitochondriaProcessing{
    ...
    call Utils.SubsetBam as SubsetBam {input:
                                           bam = bam,
                                           bai = bai,
                                           locus=locus}
    call RG_Parsing as Parsing {input:
                                    bam = SubsetBam.subset_bam}
    call Utils.BamToFastq as BamToFastq {input:
                                             bam = SubsetBam.subset_bam,
                                             prefix = prefix}

    String RG = "@RG\\tID:~{Parsing.ID}\\tSM:~{Parsing.SM}\\tPL:~{Parsing.PL}\\tPU:~{Parsing.PU}"

    call HA.Hifiasm as Hifiasm {input:
                                    reads = BamToFastq.reads_fq,
                                    prefix = prefix}
    call AR.Minimap2 as Minimap2 {input: reads = [Hifiasm.fa],
                                      ref_fasta = ref_fasta,
                                      map_preset = "map-hifi",
                                      RG = RG}
    call Quast.Quast as Quast {input:
                                   ref = ref_fasta,
